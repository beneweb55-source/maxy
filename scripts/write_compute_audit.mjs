...
// Import analyzer logic directly from our source
// Wait, we can't easily import from lib/migration/moteur.ts without TypeScript compilation or tsx.
// But we can just use tsx to run the script instead of node.
...
async function main() {
  ...
  // RE-RUN ANALYZER ON LEGACY DATA
  // We recreate what the engine did during auto-migration
  const legacyToModelMap = new Map(); // modelName => array of { categorieLegacy, referenceLegacy, nbProduits }
  ...
  for (const p of produits) {
    ...
    if (possibles.length === 1) {
      ...
      // Multiple legacy references mapped to the same model!
...
- Produits correctement conservés (non touchés car ils n'avaient pas été migrés) : **\${nbConserves}**
- **Produits généralisés par l'écrasement : \${nbGeneralises}**

Sur ces \${nbGeneralises} produits qui ont perdu leur affichage précis, voici ce que nous pouvons récupérer :
...
- **Niveau C (AMBIGU - Fusions multiples)** : \${ambigusC.length} produits
  *(Le moteur a fusionné plusieurs anciennes références différentes dans un seul et même modèle. Il est impossible de les différencier avec 100% de certitude)*
...
fs.writeFileSync("scripts/compute_audit2.ts", scriptContent);

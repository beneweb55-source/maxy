...
import { IconeBillet, IconeAlerte, IconeEtiquette } from "@/components/icons";
  ...
  useEffect(() => {
    if (unites.length > 0) {
      ...
      for (const u of unites) {
        map[u.id] = u.prix_vente_fixe ?? u.prix_vente_reel ?? (u.prix_achat > 0 ? Math.round(u.prix_achat * 1.25) : 0);
      }
      setPrixMap(map);
      setEspecesRecues("");
  ...
  return (
    ...
    >
      <div className="space-y-4 max-h-[80dvh] overflow-y-auto pr-1">
        {/* Liste des articles avec ajustement de prix */}
        <div className="space-y-2">
          ...
          <div className="max-h-40 overflow-y-auto space-y-2 rounded-xl bg-brand-light-grey/25 p-3 text-xs">
            {unites.map((u) => (
              <div key={u.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2 bg-white dark:bg-zinc-800 rounded-lg shadow-2xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="libelle mb-1.5" htmlFor="type-facture-inv">
              Type de Facture
            </label>
            <select
              id="type-facture-inv"
          ...
          <div>
            <label className="libelle mb-1.5" htmlFor="mode-paiement-inv">
              Mode de Paiement
            </label>
            <select
              id="mode-paiement-inv"
              value={modePaiement}
        ...
        {modePaiement === "especes" && (
          <div className="p-3.5 rounded-xl bg-brand-light-grey/25 border border-brand-light-grey/60 space-y-2">
            <label className="libelle text-xs" htmlFor="especes-recues-inv">
              Montant Reçu en Espèces (DA)
            </label>
            <div className="flex items-center gap-3">
              <input
                id="especes-recues-inv"
                type="number"
                value={especesRecues}
        ...
        {articlesSansEtiquette.length > 0 && (
          <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-2.5">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="toggle-etiquette-inv" className="text-xs font-bold text-amber-900 dark:text-amber-200 cursor-pointer select-none">
                Avez-vous imprimé et collé l&apos;étiquette sur les articles ({articlesSansEtiquette.length}) ?
              </label>
              <input
                id="toggle-etiquette-inv"
                type="checkbox"
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label className="libelle mb-1.5" htmlFor="client-nom-inv">
          ...
          <div>
            <label className="libelle mb-1.5" htmlFor="client-tel-inv">
              Téléphone
            </label>
            <input
              id="client-tel-inv"
          ...
          <div>
            <label className="libelle mb-1.5" htmlFor="date-vente-inv">
              Date de vente
            </label>
            <input
              id="date-vente-inv"
          ...
          <div>
            <label className="libelle mb-1.5" htmlFor="canal-inv">
              Canal de Vente
            </label>
            <input
              id="canal-inv"
        ...
        <details className="group">
          ...
          <div className="mt-2.5 grid grid-cols-1 sm:grid-cols-2 gap-2.5 p-3 bg-brand-light-grey/20 rounded-xl text-xs">
            <div>
              <label className="libelle mb-1" htmlFor="client-adresse-inv">Adresse</label>
              <input id="client-adresse-inv" type="text" value={clientAdresse} onChange={(e) => setClientAdresse(e.target.value)} className="champ" />
            </div>
            <div>
              <label className="libelle mb-1" htmlFor="client-rc-inv">RC</label>
              <input id="client-rc-inv" type="text" value={clientRc} onChange={(e) => setClientRc(e.target.value)} className="champ" />
            </div>
            <div>
              <label className="libelle mb-1" htmlFor="client-nif-inv">NIF</label>
              <input id="client-nif-inv" type="text" value={clientNif} onChange={(e) => setClientNif(e.target.value)} className="champ" />
            </div>
            <div>
              <label className="libelle mb-1" htmlFor="client-nis-inv">NIS</label>
              <input id="client-nis-inv" type="text" value={clientNis} onChange={(e) => setClientNis(e.target.value)} className="champ" />
            </div>
            <div className="sm:col-span-2">
              <label className="libelle mb-1" htmlFor="client-ai-inv">Article d&apos;imposition</label>
              <input id="client-ai-inv" type="text" value={clientAi} onChange={(e) => setClientAi(e.target.value)} className="champ" />
            </div>
          </div>
        </details>

    ...
    <div className="inline-block text-left">
      ...
      {monte && ouvert && transitions.length > 0 && createPortal(
        <div className="fixed inset-0 z-[9999] pointer-events-auto">
          ...
          <div 
            ...
            className="fixed rounded-xl border border-slate-300 bg-white dark:bg-zinc-900 shadow-2xl p-1.5 text-xs animate-entree text-slate-800 dark:text-slate-200 z-[10000]"
          >
              ...
              {transitions.map((cible) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-2 px-2.5 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors text-left font-bold group border border-transparent hover:border-slate-200 dark:hover:border-zinc-700 cursor-pointer"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-brand-orange shrink-0" />
                      <div className="min-w-0">
                        <span className="block truncate text-slate-800 dark:text-slate-100">{regleCible.libelle}</span>
      ...
      {monte && modalTransition && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 bg-black/20 backdrop-blur-sm animate-entree">
          <div 
            onClick={(e) => e.stopPropagation()}
            className="w-full max-w-md max-h-[85vh] flex flex-col bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-800 shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100"
          >
            {/* Header modal */}
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3.5 text-xs">
              ...
              {(modalTransition === "ok" || modalTransition === "a_reparer") && (
                <div>
                  ...
                  <input
                    ...
                    placeholder="0 DA"
                    className="w-full h-10 px-3 bg-white dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-xl font-mono font-bold text-xs focus:border-brand-orange focus:ring-1 focus:ring-brand-orange/30 outline-none"
                  />
                </div>
              )}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 bg-white dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-xl font-medium text-xs focus:border-brand-orange focus:ring-1 focus:ring-brand-orange/30 outline-none resize-none"
                />
              </div>
            </div>

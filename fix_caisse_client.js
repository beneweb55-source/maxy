...
code = code.replace(
  'graphique_soldes: { mois: string; solde: number }[];',
  'graphique_soldes: { jour: {label: string, solde: number}[], mois: {label: string, solde: number}[], an: {label: string, solde: number}[] };'
);

const oldGraph = `function GraphiqueLigne({ series }: { series: { mois: string; solde: number }[] }) {
  if (series.length === 0) {
    return <p className="mt-3 text-sm text-brand-warm-grey">Aucune donnée.</p>;
  }
  const largeur = 480;
  const hauteur = 180;
  const margeBas = 22;
  ...
  const y = (v: number) =>
    hauteur -
    margeBas -
    ((v - minimum) / Math.max(1, maximum - minimum)) * (hauteur - margeBas - margeHaut);
  const points = series.map((s, i) => \`\${i * pas},\${y(s.solde)}\`).join(" ");

  function libelleMois(cle: string): string {
    const [annee, mois] = cle.split("-");
    return new Date(Date.UTC(Number(annee), Number(mois) - 1, 1)).toLocaleDateString("fr-FR", {
      month: "short",
      timeZone: "UTC",
    });
  }
  ...
  return (
    <div className="mt-3 overflow-x-auto">
      <svg viewBox={\`0 0 \${largeur} \${hauteur}\`} className="h-44 w-full min-w-[320px]" role="img">
        ...
        {series.map((s, i) => (
          <g key={s.mois}>
            <circle cx={i * pas} cy={y(s.solde)} r={3.5} fill="#1770E5" />
            <text x={i * pas} y={y(s.solde) - 8} textAnchor="middle" fontSize={10} fill="#2E2D2D">
              {formaterDA(s.solde)}
            </text>
            <text x={i * pas} y={hauteur - 5} textAnchor="middle" fontSize={11} fill="#7C7572">
              {libelleMois(s.mois)}
            </text>
          </g>
        ))}
...
code = code.replace(/<GraphiqueLigne series=\{donnees.graphique_soldes\} \/>/g, '<GraphiqueLigne donnees={donnees.graphique_soldes} />');

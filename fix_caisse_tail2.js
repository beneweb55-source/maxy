...
const targetIndex = code.indexOf('function GraphiqueLigne(');
if (targetIndex !== -1) {
  // Let's just strip everything from `function GraphiqueLigne(` to the end of file
...
// wait, I also need to make sure I strip any remaining formatLabel
const formatLabelIdx = code.indexOf('function formatLabel(');
if (formatLabelIdx !== -1) {
  code = code.substring(0, formatLabelIdx);
}

fs.writeFileSync('components/caisse/CaisseClient.tsx', code + newEnd);
